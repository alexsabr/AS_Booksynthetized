...
Network which allows devices very close to communicate together,
NFC, Bluetooth

#### LAN (Local Area Network)
Devices are located in close proximity, same room, same house, same building
Wi-Fi, Ethernet Cable, Power Line Communication.
There also exists VLAN which allows separating traffic on a same Physical LAN 
...
2 ISPs client Point of Presence aggregating traffick in his neighbourhood.
3 Internal network of the ISP 
4 Backbone (Massive networking infrastructure spanning across long distances)
5 IXP (Internet eXchange Point), where two or more ISPs are interconnected to exchange packets 
...
yet the network must keep offering its service.
...
#### Safety
Network must not be diverted from its purpose 
...
It relies on services provided by the layer below it.
The same service can be provided by different protocols. 
(for example Wi-Fi and low band radio both provide the service of moving your bytes from point A to point B without a need for a wire.)
...
This is because the email app is at the top most part of the layer system, far away from the layer occupied by fiber optics / Wi-Fi .... 
Complementary, fiber optics / Wi-Fi will not discriminate bytes of a video or an email.
...
##### Connection
- Connection-full service locks resources needed to speak to its distant counterparts and keeps them locked until the resource is released,
...
#### OSI Model
This model was designed after TCP/IP and by different people,
...
- Presentation (obsolete)
  Ensure data sended to the remote application is understandable by said remote application.
- Session (obsolete)
...
- Hardware
  ...
  DSL/Color wave used in fiber optic/2.5Ghz modulation in Wi-Fi

This model has some flaws 
- It was designed when TCP/IP was already widely (for the time) implemented
  meaning strong leaning against its implementation as a lot of money has already been spent
 (For a good norm to emerge the book states David Clark from the MIT the "apocalypse of the two elephant"
  A good norm is implemented after many researches on a field has been done, yet before much money has been invested, OSI was designed when TCP/IP was invested in under the form of router and implementation in computer OS)
...
- unlike OSI, implementation and specification are intermangled
...
TCP/IP provide a working implementation that roughly resembles OSI
...
worldwide NGO promoting International standards.
...
Promote norms and research for electronics and electrics.
...
NGO responsible for maintaining name spaces on the web.
...
this part really does not interest me, and I've felt free to oversee many pages about the physical Layer.
...
#### Magnetic bands 
An LTO-7 magnetic band can carry 800 Gos of data.
...
Ethernet Cables are also using twisted pairs cables, made of various number of pairs of wires.
...
States have sliced the spectrum and sell parts of it for its usage by 
...
(where we find among other things, Wi-Fi) or the radio amateurs slices.
Here is an example of such spectrum slices :
https://en.wikipedia.org/wiki/File:United_States_Frequency_Allocations_Chart_2016_-_The_Radio_Spectrum.pdf

Some frequencies are particularly useful, as they bounce on the ionosphere and can be heard
...
Some frequencies allow to exploit the earth curvature and ionosphere as a reflector to 
extend range. 
...
##### Light 
An example of application is a laser paired with a receiver placed further away.
...
Those decompositions are called "Fourier Series"
...
which aren't attenuated. **NOT TO CONFUSE WITH ITS HOMONYM WHICH DESCRIBE HOW MANY BYTES CAN BE TRANSFERRED IN A SECOND.**
...
Later on, this method of transmitting was abandonned for instead packet switching. Here the voice of the user is digitalized and transmitted in small chunks, **packets** in a digital signal. If this was at first true only for the most dense and sollicitated part of the network,
...
Data packets follow: the same route 
- commuted circuits : Yes 
...
- Using a whole byte as a starting / ending flag delimiting the frames. The issue of this technique is that you must also allocate another byte
  ...
  If the pattern of an escaping byte appears in the data, another escaping byte is inserted before to tell that the escaping
...
- Asking the physical layer to send "illegal" bit patterns which are easily recognizable that they carry no data
  (strongly dependent on how the physical layer works)

### Error Control 
The physical layer is not perfect, errors can happen, and the signal can be altered,
a few examples of interferences sources :
...
the hamming distances between one legal word and the other, noted d is big (in a utopic word d=n)
...
##### Piggy Backing
If the receiver only send acknowledgments of received frame to an emitter on a separate simplex channel,
...
is reclaimed. One caveat though, you need to be ready to send acknowledgments if no data for the emitter is presentend in a timely fashion,
...
The sender can send frames as fast as it wishes as long as it does not overshoot the end of the frame.
...
and to deny the older ones. They "slide the window" further.
If a modulo system is used where 
In a system where the sender sends only one packet, then waits the acknowledgment of the receiver 
...
- Each entity is independant from other entities: and has its own will wether it wishes to emit traffic or not.
...
- Single Canal : All communications happen on the studied canal, where entities send and receive messages. If it wasn't the case,
  synchronization would be much easier. (Some electronic communication protocol have canal dedicated only to synchronization, with a "clock line" like I2C for example)
- Collisions of frames are witnessable : entities can detect that two or more frames from different entities are on the same channel at the same time, jamming each other. This is hypothesis very important, as protocols must adapt heavily when this hypothesis changes.
...
of all messages are correctly transmitted. Later the time was split into slots, reducing the possibility for collisions
...
- In the bitmap protocol, a "table" passes where each entity notifies if it wishes to transmit a frame.
  Afterwards each entity transmits its frame before waiting for everyone which registered in the table To do the same.
...
- In the binary counting protocol which works if the transmission time is negligeable, when the deciding phase begin,
  ...
  emits its frame, before the whole link enters a new adress emitting phase.
...
The target B answers positively **CTS** or **Clear to Send**. The entity A now knows it has the right to emit its frame. 
...
it does nothing. The emitters of the RTS will understand that something bad happened and will retry after waiting a random interval of time.
...
One solution is that if C does not hear the CTS of A nor knows A, it can assume that A is out of its reach and emit to D.
...
With the rise of the power of electronics alongside the fall of its price, ethernet hubs, 
which repeats the signal it receives along all of it ports, making a network behave like a single big ethernet cable
were replaced by ethernet switches, which tranmits ethernet packets only to the relevant port, segmenting the network and limiting its congestion.
This has been the downfall of "point to many" ethernets, toward a "point to point" architecture (even if the other point is a switch and not an end device !).
...
CSMA/CA is exploited to split the avaiable slots among stations. Since Wi-Fi is very used by nomad devices,
...
### Bluetooth
First available to the public in 1998, Bluetooth owes its name and logo to Harald Gormsson, a Danish and Norwegian King.
...
are bridging two ethernet networks. An ethernet hub is not an example of bridge, as it does not link two different ethernet networks but rather unifies them into a single one.
...
Every few minutes, stale entries in the table of what devices lie on which port is freed from old entries, so if A is unplugged on another port,
...
When the first bridge receives a packet for a recipient it does not know, it will emit it to all of it's port, excepted the on it received the packet on.
The other bridge, will receive the packet from the first bridge on a port, and if it also does not know where the recipient lies, will send it to all its port
except the one where it received the packet. Now the first bridge will receive the packet from the second bridge, and since it still does not know on what
...
### Virtual Lans
For many reasons, you may want that architecture at the Level 2 Link layer differs from the real architecture (Level 1 physical layer).
...
- Guarantees of Quality of Service
   connection-full : Easy to guarantee, ressources only have to be allocated beforehand
...
We luckily have a few algorithms to solve this problem : for example the Dijkstra Algorithm and the bellman-Ford Algorithm. The Dijkstra algorithm solves all shortest route from anywhere toward anywhere for all nodes in a graph.
...
routing loop where the same packet circles around a number of same routers indefinetly, for example a life expectancy for the packet taking 
...
This algorithm in its basic form has a regrettable flaw, if good news, as a new shortcut appeared which speeds up the routing, travel fast among the network.
...
If the link between C and D is broken, C will anounce that it no longer knows how to go to D, C has an infinite number of jumps toward D.
The problem lies in the fact that B will see this, and sees that A broadcasts that it knows a way to reach D in 3 jumps.
Not knowing that the route that A broadcasts is stale and goes through B itself, B will then update its distance vector and set A as the go-to neighbour to reach D.
A will see that B increased its distance and will consequently update its own vector to take the change broadcasted by B into account.
...
- 3. Broadcast a compiled version of all of what you have discovered to all your neighbours, and receive similar packets from your neighbours
  ...
  To avoid Congestion of the network by information packets each packet contains a sequence number.
  At any time when an information packet is received by a router, if the sequence number of said packet is lower than the
  highest previously received from the same sender, the packet is dropped, as it is a stale duplicate. Otherwise, it is broadcasted to all links except the one where it came from. Packet AND their information can also become stale from old age, so if a router restarts or a corrupted sequence number goes unnoticed, or a packet starts looping around for any reason, after a while the information it broadcasted wil become stale and be removed, and the packet will no longer be broadcasted.
...
This algorithm takes for granted though that links are bidirectionals and are ranked symetrically from the sender and the receiver perspective.
...
With this method, all routers of the network only needs to remember one tree per group in the network, and they don't even have to take part in its conception, only to remember it.
...
and think that this unique super cool router is the way to many shortest paths to other device in the network,
...
Flow Control : All measures pertaining to avoiding the situation of a sender sending too many packets for a receiver to receive,
causing packet loss.

Congestion Management : All measures pertaining to avoiding the situation of Sender sending too many packets for the n
...
- Traffic based routing : Changing the appeal of links based on how much traffic is currently using it
...
- Explicit Congestion Notification (ECN) : tagging a packet before it is delivered, so the receiver detects the impeding congestion and notifies the sender to reduce the flow of packets



#### Load Shedding :
Really the nuclear option of routers. Which packets to drop ?  It depends on the traffic. 
If you are transferring a file, if you delete the oldest packets first (example with sequence number 6) then the receiver 
will have to keep the newest packets (example sequence number 7,8,9) in a heap until the oldest had been transferred again, potentially worsening the situation. If you are transferring realtime data like Voice, 
...
How does the network layer manage when multiple packets from different emitters must follow the same route ?
...
two first class packets facing one another will be treated the same.
...
otherwise this is no longer the internet. The following sections will expose problems that arise and solutions, when interconnecting multiple networks.

#### Tunneling
What to do when you want to allow two networks A and C with the same L3 network protocol
...
To establish a communication between A and C, an entity on A will send data to its boundary router.
...
Virtual Private Networks (VPN) are a form of tunneling. Another one is IP to IP (for example IPv4 to IPv6).
...
A link state routing protocol (routers know about most router of the network and process the shortest path)
...
Routers outside a group cannot see the architecture inside. Routers can be groupless. There is also the backbone group 0,
...
A newly connected router broadcasts a HELLO message on all of its link.
...
This is not as simple as routing inside autonomous system, because inside you are interested in speed and efficiency.
Between autonomous system, you have political or monetary incentive to
...
Consult RFC 4271 for more information.
...
as long as there is an IP network at the end of the MPLS one.
...
- Don't Fragment bit, specifies if a packet can be fragmented or should be discarded if it is too big for the network.
...
IPv4 adresses are noted as four packets of numbers separated by points. These numbers range
...
Example, 192.168.0.0/16, this subnetwork masks encapsulate all ip adresses from 192.168.0.0 to 192.168.255.255
...
When a host wants to communicate with someone from the internet, its packets are transmitted to the router.
The router changes the IP adress and the source port of the packet from the local adress of the host to its own adress (of the router) on the internet
...
- NAT breaks the concept of layering, as it is concerned by what the layer above it does,
  ...
  (or it has to modify its NAT router to accept the new L4 protocol)

- The FTP Protocol, for file transmission, transmits multiple ports and IP to use, inside the payload of the packets.
  If a host behind a NAT sends this kind of info, it will relate to local network adresses and be unusable for the distant host.
...
- Payload Length
- next header, indicate either Transport Layer 4 being used or a following optional header with more information
  optional header could convey information about authentification (packet is encrypted), routing information,
  packet fragmentation, jumbo packet (for networks of super-computer) ....
- Hop limit, life expectancy of packet, to avoid it wandering forever
...
For an IP program to know to which L2 Link (and neighbour) give the packet to reach its destination,
wether the destination lies on the same link (multiple links joined by switches or hub also work)it uses the ARP protocol.
When the destination lies on another network (or another link, with at least a router between them) the ARP protocol alone won't suffice.

When the ARP protocol suffice, the host can 
- Look up in a preconfigured ARP table what is the MAC address to send the packet 
...
- configure an ARP proxy among the router on the same link as the emitter. This router
  will say that it has the IP adress that emitter wants (when it is not the case) and give its own MAC address.
...
ARP can be spoofed, an attacker can, just like an ARP proxy, broadcast its own MAC adress as holding someone's else IP,
...
The hosts make an L2 Broadcast asking for a DHCP server to provide IP address.
...
- LISTEN : Block until someone tries to connect to you
...
Host B chooses a sequence number and sends it to host A with an acknowledgment that 
Host B answers that it has acknowledged the sequence number chosen of the first host and also joins
its own sequence number to enumerate of the packets.
Host A answers that it has received the acknowledgment alongside its first packet of data. 
...
The solution to this is twofold : first introduce the notion of timer,
if after a certain amount of time no packets are received, or a disconnection request is not acknowledged,
...
and that the sender does not overflow the receiver. It also works with the Network Layer to not congest the network.
...
has error detection, or that packets won't be interverted on the network.
...
This situation translates that whe have __Maximised__ the size of the __minimum__ flow rate (the smallest flow is as big as it can get.)
...
- adopt a behavior which minimizes the time spent in congestin the network, while maximising the data rate, this is what **AIMD** is for
...
This is all assuming that the flow rate won't be too big for the receiver.
...
the arguments of the function you call, which will point to something else entirely on the remote system ? Or even just is goal to reach worth the slowdown ? (tens or hundreds of milliseconds versus nanoseconds) A thing of distributed architecutre and microservices really.
...
This mechanism is not self-sufficient. IF for any reason acknowledgments are retained for a variable period of time
(for example because the receiver uses the Nagle Algorithm), the algorithm falls apart, or its efficiency is greatly reduced.
...
This acknowledgment says that the last received packet is x-1 and that the next expected packet is x.
We can consider that after three duplicate acknowledgment (four acknowledgment received in total)
...
an acknowledgment that packet x is arrived and x+1 is expected.
...
being reopened with slow start. But we can do better by exploiting the duplicated acknowledgments as an ACK clock.
...
Cumulative acknowledgment on steroids, you can acknowledge ranges of values. The sender
...
To avoid this a few solutions : wait until there is much data to send (slow emitter) or
...
Let's take two hosts connected by a one Gigabit per second fiber optic with an RTT of 50 miliseconds.
...
Email are exchanged as follows, the agent user of the sender connects to its message server, to transmit an email it wishes to d
...
When the agent user will next connect to its message server, the message server will notify it that 
...
MIME is today used not only in emails but also in web development.
...
3xx Tells the browser to redirect its request to someone else
...
information about it like account automatic reconnection, it's shopping cart, ....
...
This code will call the website of the announcer and allow it to deposit its own cookie, the third party cookie.
If the user then browses toward another website which has also signed an agreement with the same advertisement company
the same scenario will repeat, allowing the advertisement company to access the cookie stored earlier, and thus, extracting
information about the browsing of the other website. The advertisement company can also store other information,
...
Browser allow website, for performance reason, a variety of information about the device
...
which determines if with fingerprinting technologies only if you can be isolated 
...
content. They can both be client and host at the same time, giving its name to the protocol
from one peer, to another peer.
The first P2P widespread service was Napster, its flaw was a centralized database of content.
It caused its downfall since the service became well known for illegally sharing music,
...
To answer the third question, regularly, peers evaluate who tries its best to share shunk (seeders)
and who tries its least, while trying to download as much as possible (leechers).
...
## Security
The first hack goes through network goes back the 1960s era,
...
To reach the objectives of the security triangles, there are seven principles to guide
our hand
...
-4 Principle of least Authority
  Any system or user shall be given the least amount of power / authority which it needs to perform its tasks.
  By doing so, if a user or system is compromised, the whole system does not automatically crumple like a house of cards.
...
an attack
- Recon
  First you need to obtain as much information as possible about your target, without
  ...
  - going near (not too much) the physical location of the target and inspecting if
    something could be exploited (easily accessible networking cabinet, information about hardware used, wireless communications ...)
- sniffing and snooping
  Like recon but maybe a bit more involved, and also may get you spotted. You're looking for
  firewalls, IP adresses, open connections, hardware information, some L2 packets, useful DNS entries etc.
- spoofing
...
- Disturbing
  ...
  this third party will then reroute traffick inside its Autonomous System toward yours.
...
information but also because it "understands" the L7 Application used (or the content).
...
Too many false negative and the system is useless. Too many false positives will tire you,
...
- First a part of the data must be **redundant**, that is it must include some information that is not
...
A bulletproof technique. Yes really ! Because from the ciphered text,
...
Which permutation / substitution is applied is depending on the key. Reminder, permutation is changing the position of the bits, substitution is changing the value of the bits.
...
today deemed obsolete and weak it is no longer used. The same applies for its variant the "Triple-DES".
...
- Symetric cipher, one block of data after the other 
...
Two agent that wish to communicate with this kind of algorithm proceed as follows,
...
and reaching the correct recipient (provided there is no Man In the Middle when the public key were exchanged.)
...
It exposes its own public key to both agents, so it
can decipher their communication, before forwarding it to the other agent.
Both agents cannot realize that they are being listened upon.
...
This is where certificates come to place.
...
its goal is to prove that both parties
...
A establishes communcation with B, and sends a message containing a challenge that only someone possessing the key of A can create and resolve,
...
This protocol provides "à la carte" security,
...
### Wi-Fi Security
One of the quality of Wi-Fi, allowing remote devices to communicate,
is also a security defect. Since Wi-Fi signal can be far-reaching,
an attacker can simply place itself not far from it's target, but in the public space,
and listen for Wi-Fi packets to intercept.
Consequently, Wi-Fi communications needs to be protected, but we are going to see that 
this is sometimes lacking.

#### Wired Equivalent Privacy (WEP)
The first security algorithm for Wi-Fi connections.
...
#### Wi-Fi Protected Access 1,2,3 (WPA)
The evolutions of Wi-Fi security algorithm after WEP,
...
Many encryption algorithms are now supported like AES-256 and RSA. 
...
Clients aren't authenticated by SSL/TLS, only the server is. It is at the will and the responsability of the server
...
where an attacker makes the user's hardware execute dangerous code that is not originating from the website
...
Users may also want to extend the capabilites of their web browser. They can use browser extensions.
...
user experience, and gigantic security concern led to its discontinuation 
